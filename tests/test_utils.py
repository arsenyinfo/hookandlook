...
def test_stats_reservoir():
    ...
    for _ in range(5):
        reservoir.add({'value': 1, 'input_name': 'name', 'stat_name': 'mean', 'module_name': '', 'is_training': True})

    assert len(reservoir) == 5

    for _ in range(10):
        reservoir.add({'value': 0, 'input_name': 'name', 'stat_name': 'mean', 'module_name': '', 'is_training': True})
